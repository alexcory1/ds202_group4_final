...
ggplot(data=df1, aes(x=df1$date, y=proportion_released, color=CaseDelta)) +
  ...
  scale_x_date(date_labels = "%m-%y",
               date_breaks = "2 month") +
  ggtitle("Proportion released by month")
  theme(axis.text.x = element_text(angle = 45, hjust=1))
...
ggp1 <- ggplot(data = df1, aes(x=df1$date)) +       
  geom_line(aes(y = df1$genMale), color = "lightblue") +
  geom_line(aes(y = df1$genFemale), color = "pink") +
  labs(x="Date", y="Cases") +
  ggtitle("Cases by Gender (Female corresponds to pink, male to blue)") +
  theme(axis.text.x = element_text(angle = 45, hjust=1))
