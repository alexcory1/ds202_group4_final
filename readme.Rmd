...
library(data.table)
df <- fread('~/ds202_group4_final/MD_COVID-19_-_MASTER_Case_Tracker.csv')
show(df)
```
...
```{r}
ggplot(data=df1, aes(x=df1$month, y=df1$TotalCases, color=CaseDelta)) +
  facet_grid(. ~df1$year)+
  xlab("Month") +
  ylab("Total Cases") +
...
ggplot(data=df1, aes(x=df$ReportDate, y=proportion)) +
  facet_grid(. ~df1$year)+
  xlab("Report Date") +
  ylab("Proportion") +
  geom_point()
